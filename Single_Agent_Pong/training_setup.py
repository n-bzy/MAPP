...
def hyperparameter_settings(num_actions = 6, ERP_size = 75000, EPISODES = 20_000, 
                            epsilon = 1,
...
def create_env(name_env = 'ALE/Pong-v5'):
    ...
    env = gym.make(name_env, frameskip=1)
    env = gw.AtariPreprocessing(env, noop_max=30, frame_skip=4, screen_size=84, terminal_on_life_loss=False, grayscale_obs=True, grayscale_newaxis=True,scale_obs=False)
